package io.github.treesitter.jtreesitter.crab;

import java.lang.foreign.*;

public final class TreeSitterCrab {
    ...
    private static final TreeSitterCrab INSTANCE = new TreeSitterCrab();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterCrab() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_crab");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-crab");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-crab");
